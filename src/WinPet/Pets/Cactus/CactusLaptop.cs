...
internal static class CactusLaptop
{
    public static void Paint(Graphics g, double seconds, float amount)
    {
        ...
        using var edge = new Pen(CactusPalette.Edge, 1.6f) { LineJoin = LineJoin.Round };
        using var shell = new SolidBrush(Color.FromArgb(161, 183, 179));
        using var deck = new SolidBrush(CactusPalette.PropHighlight);
        ...
        using var highlight = new Pen(CactusPalette.PropHighlight, 1.2f);
