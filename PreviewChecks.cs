...
// Run by the generator, so broken/missing animation artifacts fail the build.
internal static class PreviewChecks
{
    public static void Verify(string directory, IEnumerable<string> sceneIds, int frames, int delay)
    {
        foreach (string id in sceneIds)
        {
            ...
            if (still.Size != new Size(76, 92) || gif.Size != new Size(240, 208) || gif.GetFrameCount(FrameDimension.Time) != frames)
